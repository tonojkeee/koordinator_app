...
from app.modules.chat.validators import (
...
async def create_channel(
    ...
    if current_user.role != "admin":
        ...
        if str(allow_create).lower() != "true":
            raise HTTPException(
                status_code=403, detail="Создание каналов запрещено администратором"
            )
        ...
        await manager.broadcast_to_all_users(
            {
                ...
                "channel": {
                    ...
                    "other_user": None,
                    "created_at": enriched_channel.created_at.strftime(
                        "%Y-%m-%dT%H:%M:%SZ"
                    ),
...
async def join_channel(
    ...
    if channel.is_direct:
        raise HTTPException(
            status_code=403, detail="Нельзя присоединиться к приватному чату"
        )
...
async def transfer_channel_owner(
    ...
    if channel.created_by != current_user.id:
        raise HTTPException(
            status_code=403, detail="Только владелец может передать права"
        )
    ...
    if not is_new_owner_member:
        raise HTTPException(
            status_code=400, detail="Новый владелец должен быть участником канала"
        )
...
async def leave_channel(
    ...
    if channel.is_direct:
        raise HTTPException(
            status_code=403, detail="Нельзя покинуть личный чат (удалите его)"
        )
    ...
    if not success:
        raise HTTPException(
            status_code=400, detail="Вы не являетесь участником этого канала"
        )
        ...
        result.append(
            MessageWithUser(
                ...
                **doc_info,
                ...
                parent=(
                    msg.parent_info
                    if hasattr(msg, "parent_info") and msg.parent_info
                    else None
                ),
            )
        )
...
async def get_message_replies(
    ...
    is_member = await ChatService.is_user_member(
        db, parent_message.channel_id, current_user.id
    )
    if not is_member:
...
async def mute_channel(
    channel_id: int,
    mute_until: Optional[str] = Query(
        None, description="ISO format datetime string, or null to unmute"
    ),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    ...
    success = await ChatService.set_mute_status(
        db, channel_id, current_user.id, mute_dt
    )
    if not success:
        # Auto-join if public? Logic implies user must be member.
        # For now, just error if not member.
        raise HTTPException(
            status_code=404, detail="Channel not found or you are not a member"
        )
...
async def add_reaction(
    ...
    is_member = await ChatService.is_user_member(
        db, message.channel_id, current_user.id
    )
    if not is_member:
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not user or not user.is_active:
                logger.warning(
                    f"User {user_id} is inactive or not found, rejecting connection"
                )
                return
            ...
            if existing_connections >= 5:
                logger.warning(
                    f"User {user_id} has too many connections ({existing_connections}), rejecting"
                )
                return
...
async def websocket_endpoint(
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not user or not user.is_active:
            await websocket.close(
                code=status.WS_1008_POLICY_VIOLATION, reason="Account disabled"
            )
            return
    ...
    try:
        while True:
            ...
            async with AsyncSessionLocal() as db:
                is_current_member = await ChatService.is_user_member(
            ...
            async with AsyncSessionLocal() as db:
                max_len_setting = await ConfigService.get_value(
                    db, "chat_max_message_length"
                )
                try:
            ...
            if mentioned_usernames:
                async with AsyncSessionLocal() as db_session:
                    mention_stmt = select(User).where(
            ...
            async with AsyncSessionLocal() as db:
                ...
                message = await ChatService.create_message(
                    db, message_data, user_id, document_id=document_id
                )
                ...
                if message.parent_id:
                    ...
                    if parent_msg:
                        parent_info = {
                            ...
                            "username": (
                                parent_msg.user.username
                                if parent_msg.user
                                else "Unknown"
                            ),
                            "full_name": (
                                parent_msg.user.full_name if parent_msg.user else None
                            ),
                        }
                ...
                member_ids = await ChatService.get_channel_member_ids(db, channel_id)
                logger.info(
                    f"New message in channel {channel_id}. Broadcasting to members: {member_ids}"
                )
...
async def start_direct_message(
    ...
    existing_channel = await ChatService.get_or_create_direct_channel(
...
async def delete_message(
    ...
    if not is_admin:
        ...
        if str(allow_delete).lower() != "true":
            raise HTTPException(
                status_code=403, detail="Удаление сообщений запрещено администратором"
            )

    deleted_message = await ChatService.delete_message(
        db, message_id, current_user.id, is_admin
    )
    ...
    if deleted_message.document_id:
        try:
            ...
            if document:
                ...
                try:
                    safe_path = safe_file_operation(
                        document.file_path, "uploads/documents"
                    )
...
async def update_message(
    ...
    await manager.broadcast_to_channel(
        full_msg.channel_id,
        {
            ...
            "content": full_msg.content,
            "updated_at": (
                full_msg.updated_at.strftime("%Y-%m-%dT%H:%M:%SZ")
                if full_msg.updated_at
                else None
            ),
        },
    )
...
async def export_chat_history(
    ...
    buffer.write(
        f"Экспортировал: {current_user.full_name or current_user.username} ({current_user.username})\n"
    )
...
@router.post(
    "/invitations",
    response_model=ChannelInvitationResponse,
...
async def accept_invitation(
    ...
    channel = await InvitationService.accept_invitation(
        db, accept_data.invitation_id, current_user.id
    )
...
async def decline_invitation(
    ...
    await InvitationService.decline_invitation(
        db, decline_data.invitation_id, current_user.id, decline_data.reason
    )
...
async def get_channel_invitations(
    ...
    return await InvitationService.get_channel_invitations(
        db, channel_id, current_user.id, include_expired
    )
