...
class ChatService:
    ...
    async def create_notifications_channel(db: AsyncSession, user_id: int) -> Channel:
        ...
        channel_member = ChannelMember(
            channel_id=notifications_channel.id, user_id=user_id, role="owner"
        )
    ...
    async def create_channel(
        db: AsyncSession, channel_data: ChannelCreate, user_id: int
    ) -> Channel:
    ...
    async def get_or_create_direct_channel(
        db: AsyncSession, user_id1: int, user_id2: int
    ) -> Channel:
    ...
    @staticmethod
    async def get_or_create_notifications_channel(
    ...
    async def get_user_channels(db: AsyncSession, user_id: int) -> List[Channel]:
        ...
        notifications_channel = await ChatService.get_or_create_notifications_channel(
            db, user_id
        )
        ...
        result = await db.execute(
            ...
            .where(
                or_(
                    ...
                    and_(Channel.is_system == True, ChannelMember.user_id == user_id),
    ...
    async def get_channel_messages(
        db: AsyncSession, channel_id: int, limit: int = 50, offset: int = 0
    ) -> List[Message]:
        ...
        result = await db.execute(
            ...
            )  # Removed parent_id.is_(None) check to show all messages
    ...
    @staticmethod
    async def get_replies(db: AsyncSession, parent_id: int) -> List[Message]:
    ...
    async def add_member(
        ...
        member = ChannelMember(
            channel_id=channel_id, user_id=user_id, last_read_message_id=latest_id
        )
        db.add(member)
        await db.commit()
    ...
    async def transfer_channel_ownership(
        db: AsyncSession, channel_id: int, new_owner_id: int
    ) -> bool:
    ...
    async def add_reaction(
        ...
        stmt = (
            ...
            .where(MessageReaction.id == reaction.id)
        )
        result = await db.execute(stmt)
        return result.scalars().first()
    ...
    async def toggle_pin_channel(
        ...
        if not member:
            ...
            if channel and not channel.is_direct:
                member = ChannelMember(
                    channel_id=channel_id, user_id=user_id, is_pinned=True
                )
                db.add(member)
                await db.commit()
                return True
    ...
    async def set_mute_status(
        db: AsyncSession, channel_id: int, user_id: int, mute_until: Optional[datetime]
    ) -> bool:
    ...
    async def search_messages(
        db: AsyncSession, query: str, user_id: int, limit: int = 50, offset: int = 0
    ) -> List[Message]:
        ...
        accessible_channels = select(Channel.id).where(
            or_(Channel.id.in_(subquery), Channel.is_direct == False)
        )
