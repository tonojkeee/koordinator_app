...
class InvitationService:
    ...
    async def create_invitation(
        db: AsyncSession, invitation_data: ChannelInvitationCreate, inviter_id: int
    ) -> ChannelInvitationResponse:
        ...
        user_result = await db.execute(
            select(User).where(
                func.lower(User.email) == invitation_data.invitee_email.lower()
            )
        )
        ...
        existing_invitation_result = await db.execute(
            select(ChannelInvitation).where(
                and_(
                    ...
                    func.lower(ChannelInvitation.invitee_email)
                    == invitation_data.invitee_email.lower(),
        ...
        expires_at = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(
            hours=invitation_data.expires_hours
        )
    ...
    async def accept_invitation(
        ...
        try:
            ...
            member = ChannelMember(
                channel_id=invitation.channel_id, user_id=user_id, role=invitation.role
            )
            db.add(member)
            ...
            await db.rollback()
            logger.error(
                f"Error accepting invitation {invitation_id}: {e}", exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def decline_invitation(
        db: AsyncSession, invitation_id: int, user_id: int, reason: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            await db.rollback()
            logger.error(
                f"Error declining invitation {invitation_id}: {e}", exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def get_pending_invitations(
        ...
        for invitation in invitations_result.scalars().all():
            response = InvitationService._invitation_to_response(
                invitation, invitation.channel, invitation.inviter
            )
    ...
    async def get_channel_invitations(
        db: AsyncSession, channel_id: int, user_id: int, include_expired: bool = False
    ) -> ChannelInvitationList:
        ...
        total_result = await db.execute(
            select(func.count(ChannelInvitation.id)).where(
                ChannelInvitation.channel_id == channel_id
            )
        )
    ...
    def _invitation_to_response(
        invitation: ChannelInvitation, channel: Channel, inviter: User
    ) -> ChannelInvitationResponse:
