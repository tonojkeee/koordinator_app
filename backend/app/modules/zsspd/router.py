...
async def get_outgoing_packages(
    ...
    return await service.get_packages(
        direction=ZsspdDirection.OUTGOING, skip=skip, limit=limit, user_id=user_id
    )


@router.post(
    "/outgoing", response_model=ZsspdPackageRead, status_code=status.HTTP_201_CREATED
)
async def create_outgoing_package(
    ...
    if package.direction != ZsspdDirection.OUTGOING:
        raise HTTPException(
            status_code=400, detail="Invalid direction for this endpoint"
        )
...
async def upload_package_file(
    ...
    if package.created_by != current_user.id and current_user.role != "admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to edit this package"
        )
...
async def update_package_status(
    ...
    if update_data.status in [
        ZsspdStatus.EXPORTED,
        ZsspdStatus.SENT,
        ZsspdStatus.RECEIVED,
        ZsspdStatus.DISTRIBUTED,
    ]:
        if current_user.role not in ["admin", "operator"]:
            raise HTTPException(
                status_code=403, detail="Only operators can change to this status"
            )
...
async def get_package_details(
    ...
    if package.created_by != current_user.id and current_user.role not in [
        "admin",
        "operator",
    ]:
        raise HTTPException(
            status_code=403, detail="Not authorized to view this package"
        )
