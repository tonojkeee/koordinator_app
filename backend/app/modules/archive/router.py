...
@router.post(
    "/folders",
    response_model=ArchiveFolderResponse,
...
async def create_folder(
    ...
    """Create a new folder"""
    target_unit_id = (
        folder_data.unit_id if folder_data.unit_id else current_user.unit_id
    )

    # Optional: restriction - only admins can create folders in other units (for public)
    if (
        not folder_data.is_private
        and current_user.role != "admin"
        and target_unit_id != current_user.unit_id
    ):
        raise HTTPException(
            status_code=403, detail="You can only create folders in your own unit"
        )
...
@router.post(
    "/upload", response_model=ArchiveFileResponse, status_code=status.HTTP_201_CREATED
)
async def upload_file(
    ...
    if (
        ...
        raise HTTPException(
            status_code=403, detail="You can only upload files to your own unit"
        )
    ...
    if allowed_types_str:
        ...
        if extension not in allowed:
            raise HTTPException(
                status_code=400, detail=f"File type {extension} not allowed"
            )
    ...
    if file.size and file.size > max_bytes:
        raise HTTPException(
            status_code=413, detail=f"File too large (max {max_mb_str}MB)"
        )
...
async def view_file(
    ...
    if current_user.role != "admin" and current_user.unit_id != file_record.unit_id:
        raise HTTPException(
            status_code=403, detail="Access denied to this unit's archive"
        )
...
async def delete_file(
    ...
    if current_user.unit_id != file_record.unit_id:
        raise HTTPException(
            status_code=403,
            detail="Вы можете удалять только файлы своего подразделения",
        )
...
async def delete_folder(
    ...
    if current_user.unit_id != folder.unit_id:
        raise HTTPException(
            status_code=403,
            detail="Вы можете удалять только папки своего подразделения",
        )
...
async def update_folder(
    ...
    if (
        ...
        and folder.owner_id != current_user.id
        and folder.unit_id != current_user.unit_id
    ):
...
async def update_file(
    ...
    if (
        current_user.role != "admin"
        and file_record.owner_id != current_user.id
        and file_record.unit_id != current_user.unit_id
    ):
        raise HTTPException(status_code=403, detail="Access denied")
...
async def batch_action(
    ...
    for item_id, item_type in zip(batch_data.item_ids, batch_data.item_types):
        if item_type == "file":
            ...
            if current_user.role != "admin" and file.unit_id != current_user.unit_id:
                raise HTTPException(
                    status_code=403, detail="Access denied to source files"
                )
        else:
            folder = await ArchiveService.get_folder_by_id(db, item_id)
            if not folder:
                raise HTTPException(
                    status_code=404, detail=f"Folder {item_id} not found"
                )
            ...
            if current_user.role != "admin" and folder.unit_id != current_user.unit_id:
                raise HTTPException(
                    status_code=403, detail="Access denied to source folders"
                )

    # Security: verify user has permission to write to target unit
    if (
        batch_data.target_unit_id != current_user.unit_id
        and current_user.role != "admin"
    ):
        raise HTTPException(
            status_code=403, detail="Cannot move/copy to another unit's archive"
        )
    ...
    if batch_data.action == "move":
        ...
        raise HTTPException(
            status_code=400, detail="Invalid action. Use 'move' or 'copy'."
        )
    ...
    if success:
        ...
        logger.error(
            f"Batch {batch_data.action} operation failed for user {current_user.id}. Items: {batch_data.item_ids}"
        )
        raise HTTPException(
            status_code=500,
