...
class ArchiveService:
    ...
    async def delete_folder(db: AsyncSession, folder_id: int) -> bool:
        ...
        if files:
            await db.execute(
                delete(ArchiveFile).where(ArchiveFile.id.in_([f.id for f in files]))
            )
    ...
    async def update_folder(
        db: AsyncSession, folder_id: int, folder_data: dict
    ) -> Optional[ArchiveFolder]:
    ...
    async def update_file(
        db: AsyncSession, file_id: int, file_data: dict
    ...
    async def update_file_content(
        db: AsyncSession, file_id: int, file: UploadFile
    ) -> Optional[ArchiveFile]:
        file_record = await ArchiveService.get_file_by_id(db, file_id)
        if not file_record:
            return None
        ...
        file_record.file_path = (
            f"{file_record.unit_id}/{safe_filename}"  # Relative to UPLOAD_DIR
        )
        file_record.file_size = file.size
        # Update title if it changed (usually same)
        file_record.created_at = (
            datetime.now()
        )  # Update "last modified" time if needed, though strictly it's "created_at" in our model
    ...
    async def move_items(
        ...
        if folder_ids:
            ...
            for folder_id in folder_ids:
                await ArchiveService._update_folder_children_context(
                    db, folder_id, target_unit_id, is_private
                )
    ...
    @staticmethod
    async def _update_folder_children_context(
        db: AsyncSession, folder_id: int, unit_id: int, is_private: bool
    ):
    ...
    async def copy_items(
        ...
        for item_id, item_type in zip(item_ids, item_types):
            if item_type == "file":
                ...
                await ArchiveService._copy_folder_recursive(
                    db, item_id, target_folder_id, target_unit_id, is_private, owner_id
                )
    ...
    async def _copy_folder_recursive(
        ...
        for sub in subfolders:
            await ArchiveService._copy_folder_recursive(
                db, sub.id, new_folder.id, target_unit_id, is_private, owner_id
            )
