...
async def handle_user_created(event: UserCreated) -> None:
    ...
    """
    logger.info(
        f"Creating email account for user {event.username} (ID: {event.user_id})"
    )
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            email_account = EmailAccount(
                user_id=event.user_id, email_address=event.email
            )
            db.add(email_account)
            await db.commit()
            logger.info(
                f"Successfully created email account {event.email} for user {event.user_id}"
            )
        except Exception as e:
            logger.error(
                f"Failed to create email account for user {event.username}: {e}"
            )
