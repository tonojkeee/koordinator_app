...
async def get_overview(db: AsyncSession = Depends(get_db), _=Depends(get_admin_user)):
...
async def get_activity(
    days: int = 7, db: AsyncSession = Depends(get_db), _=Depends(get_admin_user)
):
...
async def get_storage(db: AsyncSession = Depends(get_db), _=Depends(get_admin_user)):
...
async def get_active_sessions(
    db: AsyncSession = Depends(get_db), _=Depends(get_admin_user)
):
...
async def get_unit_stats(db: AsyncSession = Depends(get_db), _=Depends(get_admin_user)):
...
async def get_top_users(db: AsyncSession = Depends(get_db), _=Depends(get_admin_user)):
...
async def get_audit_logs(
    limit: int = 50, db: AsyncSession = Depends(get_db), _=Depends(get_admin_user)
):
...
async def get_system_health(request: Request, _=Depends(get_admin_user)):
...
async def get_all_tasks(db: AsyncSession = Depends(get_db), _=Depends(get_admin_user)):
...
async def get_database_status(
    ...
    return {
        ...
        "database_name": settings.database_url.split("/")[-1].split("?")[
            0
        ],  # Rough extraction
...
async def get_database_config(_=Depends(get_admin_user)):
...
async def test_database_connection(config: DatabaseConfig, _=Depends(get_admin_user)):
    ...
    if config.type == "sqlite":
        ...
        if not all([config.host, config.user, config.database]):
            raise HTTPException(
                status_code=400, detail="Missing required fields for MySQL"
            )
...
async def save_database_config(config: DatabaseConfig, _=Depends(get_admin_user)):
    ...
    if config.type == "sqlite":
        ...
        if not all([config.host, config.user, config.database]):
            raise HTTPException(
                status_code=400, detail="Missing required fields for PostgreSQL"
            )
