...
async def upload_document(
    ...
    allowed_extensions = (
        {ext.strip() for ext in allowed_types_str.split(",")}
    ...
    try:
            ...
            for chunk in iter(lambda: file.file.read(8192), b""):
                ...
                if size > max_size:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to cleanup oversized file {file_path}: {cleanup_err}"
                        )
                    raise HTTPException(
                        status_code=413,
                        detail=f"File too large (max {max_size_mb_str}MB)",
                    )
                buffer.write(chunk)
        ...
        raise HTTPException(
            status_code=500,
            detail="Не удалось сохранить файл. Проверьте права доступа к диску.",
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Произошла ошибка при загрузке файла. Попробуйте еще раз.",
        )
...
async def get_my_documents(
    ...
    return await BoardService.get_owned_documents(
...
async def get_received_documents(
    ...
    return await BoardService.get_shared_with_me_documents(
        db, current_user.id, skip=skip, limit=limit
    )
...
async def share_document(
    ...
    if document.owner_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Not authorized to share this document"
        )

    # Get or create DM channel for the share
    channel = await ChatService.get_or_create_direct_channel(
        db, current_user.id, share_data.recipient_id
    )
...
async def _validate_file_and_channel(
    file: UploadFile, channel_id: Optional[int], current_user: User, db: AsyncSession
) -> set:
...
async def _save_uploaded_file(
    file: UploadFile, extension: str, db: AsyncSession
) -> tuple[str, int]:
...
async def _share_document_with_recipients(
    ...
    if new_ids:
        shares = [
            DocumentShare(document_id=document.id, recipient_id=rid) for rid in new_ids
        ]
        db.add_all(shares)
        await db.commit()
...
async def _post_document_to_channel(
    ...
    msg = await ChatService.create_message(
        db, msg_data, current_user.id, document_id=document.id
    )
...
async def _send_document_via_dms(
    ...
    msg_content = (
        description if description else f"📎 Поделился файлом: {document.title}"
    )
    ...
    for r_id in recipient_ids:
        ...
        dm_channel = await ChatService.get_or_create_direct_channel(
            db, current_user.id, r_id
        )
...
async def upload_and_share_document(
    ...
    await _share_document_with_recipients(
        db, document, r_ids, current_user.id, notify=False
    )

    if channel_id:
        await _post_document_to_channel(
            db, channel_id, document, description, current_user
        )
    else:
