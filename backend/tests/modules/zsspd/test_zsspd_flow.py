...
async def get_auth_headers(
    client: AsyncClient, db_session: AsyncSession, username="zsspduser", role="user"
):
...
@pytest.mark.asyncio
async def test_outgoing_package_lifecycle(
    client: AsyncClient, db_session: AsyncSession
):
    """Test full outgoing package lifecycle and role restrictions"""
    user_headers, user = await get_auth_headers(
        client, db_session, "user_sender", "user"
    )
    admin_headers, admin = await get_auth_headers(
        client, db_session, "admin_operator", "admin"
    )
    ...
    response = await client.put(
        f"/api/zsspd/packages/{package_id}",
        headers=user_headers,
        json={"status": ZsspdStatus.READY},
    )
    ...
    response = await client.put(
        f"/api/zsspd/packages/{package_id}",
        headers=user_headers,
        json={"status": ZsspdStatus.EXPORTED},
    )
...
async def test_zsspd_file_upload(client: AsyncClient, db_session: AsyncSession):
    ...
    res = await client.post(
        "/api/zsspd/outgoing",
        headers=headers,
        json={"subject": "File Test", "direction": "OUTGOING"},
    )
    ...
    response = await client.post(
        f"/api/zsspd/packages/{package_id}/files", headers=headers, files=files
    )
...
async def test_zsspd_access_control(client: AsyncClient, db_session: AsyncSession):
    ...
    res = await client.post(
        "/api/zsspd/outgoing",
        headers=headers1,
        json={"subject": "User 1 Secret", "direction": "OUTGOING"},
    )
    package_id = res.json()["id"]
