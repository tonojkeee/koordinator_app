import { Hash, Users, Bell, BellOff, Info, LogOut, Search, Download } from 'lucide-react';
...
interface ChannelHeaderProps {
  channel: Channel & { is_direct?: boolean; other_user?: User; display_name?: string; name?: string; is_owner?: boolean; is_member?: boolean; members_count?: number; online_count?: number };
  isConnected: boolean;
  isMuted: boolean;
      ...
      <div className="flex items-center justify-between px-6 py-3">
        ...
        <div className="flex items-center gap-4 flex-1 min-w-0">
            ...
            {channel?.is_direct && channel.other_user ? (
              <span className="text-sm font-black uppercase">
                {channel.other_user.full_name
                  ? channel.other_user.full_name.split(' ').map((n: string) => n[0]).join('').slice(0, 2)
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-base font-black text-slate-900 truncate flex items-center gap-2">
              {channel?.is_direct && channel.other_user
              ...
              {channel?.is_owner && !channel?.is_direct && (
                <OwnerIndicator size="md" tooltip={t('chat.owner')} />
              )}
              {channel?.is_owner && !channel?.is_direct && (
                <span className="text-xs font-bold text-amber-600">({t('chat.youAreOwner')})</span>
              )}
            </h2>
